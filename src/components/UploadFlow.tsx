...
export default function UploadFlow({ onGoToLista }: Props) {
  ...
  const handleSaveValid = async () => {
    ...
    try {
      const items = validItems.map(r => {
        const { id: _id, ...rest } = r.comprobante!
        if (clienteActivo?.id) (rest as Comprobante).clienteId = clienteActivo.id
        return rest as Comprobante
      })
      ...
      await db.transaction('rw', db.comprobantes, db.lotesCarga, async () => {
        ...
        if (currentLoteId) {
          ...
          await db.lotesCarga.update(currentLoteId, {
            estado: hasErrors ? 'observado' : 'procesado',
      ...
      {step === 'upload' && (
        <div className="space-y-4 max-w-3xl mx-auto">
          ...
          >
            ...
            {error && (
              <div className="mt-4 bg-error-bg border border-error/30 text-error px-4 py-3 rounded-xl text-sm">
                {error}
              </div>
            )}
          </div>
      ...
      {step === 'batch-review' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <StatCard label="Válidos" value={summary.valid} icon={<CheckCircle size={24} />} iconBg="bg-teal/20" iconColor="text-teal" />
            <StatCard label="Duplicados" value={summary.duplicate} icon={<AlertTriangle size={24} />} iconBg="bg-yellow-500/20" iconColor="text-yellow-500" />
            <StatCard label="Errores" value={summary.error} icon={<XCircle size={24} />} iconBg="bg-error/20" iconColor="text-error" />
          </div>
                  ...
                  <tr key={item.id} className="hover:bg-white/5 transition-colors group">
                    ...
                    <td className="py-3 px-4 text-right">
                      <button
                        ...
                        className="text-teal hover:text-teal-light opacity-50 hover:opacity-100 transition-opacity p-2 cursor-pointer focus:outline-none focus:ring-2 focus:ring-teal/40 rounded"
                        aria-label="Revisar y editar"
                      >
                        <FileEdit size={16} />
                      </button>
                    </td>
      ...
      {step === 'done' && (
        <div className="bg-glass border border-glass-border rounded-2xl p-12 text-center max-w-3xl mx-auto">
          ...
          <div className="flex gap-3 justify-center">
            ...
            <Button variant="secondary" onClick={onGoToLista} size="lg">
              Volver a la lista
            </Button>
          </div>
        </div>
